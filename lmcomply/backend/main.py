...
app = FastAPI(title="LM Comply", version="3.2")
db.init()
...
@app.get("/api/health")
def health():
    return {"status": "ok", "service": "LM Comply", "version": "3.2"}
...
async def scan(file: UploadFile = File(...), px_per_mm: float | None = Form(None), demo_text: str = Form(""), category: str = Form("general"), x_role: str = Header(default="inspector")):
    ...
    # CRITICAL/MAJOR = verified or strongly indicated violation. REVIEW = extraction uncertainty.
