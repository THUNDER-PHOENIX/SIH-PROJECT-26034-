...
app = FastAPI(title="LM Comply", version="3.0")
db.init()
...
@app.get("/api/health")
def health():
    return {"status": "ok", "service": "LM Comply", "version": "3.0"}
...
async def scan(
    ...
    ctx = {"px_per_mm": ppm, "font_mm": fields["font_mm"], "ocr_confidence": fields["ocr_confidence"], "boxes": ocr["boxes"]}
    violations = rules.evaluate(fields, ctx)
...
def scans(qtext: str = Query("", alias="q"), status: str = Query(""), category: str = Query(""), limit: int = Query(100, ge=1, le=500), x_role: str = Header(default="inspector")):
    ...
    if category.strip():
        sql += " AND p.category=?"; args.append(category.strip())
